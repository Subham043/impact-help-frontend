...
export default function Index() {
  ...
  const getTicketData = useCallback(async (pg = 0) => {
    showLoader()
    try {
      const response = await axiosPrivate.get(`/ticket/view?page=${pg}`);
  ...
  const deleteTicketData = useCallback(async (id) => {
    showLoader()
    try {
      const response = await axiosPrivate.delete(`/ticket/delete/${id}`);
  ...
  const deleteSelectedTicketData = async () => {
    ...
    showLoader()
    try {
      const response = await axiosPrivate.post(`/ticket/delete-multiple`, { ids: selectedIds });
      // console.log(response.data.message)
      hideLoader()
      getTicketData(tablePaginationData.currentPage)
      getStatusData()
  ...
  const getStatusData = useCallback(async () => {
    showLoader()
    try {
      const response = await axiosPrivate.get(`/ticket/get-status-counter`);
  ...
  return (
    ...
    <InnerDashboardLayout>
      <DashboardHeader name="Dashboard" link="/create-ticket" routeName="Create New Ticket" />
      {statusData?.length > 0 && (
      ...
      <div className="main-dashboard-table-data">
        ...
        <table {...getTableProps()} className="table table-bordered table-striped table-hover table-responsive" >
          <thead>
                  ...
                  {row.cells.map((cell, i) => {
                    ...
                    if ((cell?.column?.Header) === 'Action') {
                      return (
                        ...
                        >
                          <Link to={`/view-ticket/${cell?.row?.values?.id}`} className="viewBtn"><FaRegEye /></Link>
                          <Link to={`/edit-ticket/${cell?.row?.values?.id}`} className="editBtn"><AiOutlineEdit /></Link>
