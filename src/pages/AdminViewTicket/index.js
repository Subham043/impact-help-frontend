...
const Index = () => {
    ...
    const getTicketUpdateData = async () => {
        showLoader()
        try {
            const response = await axiosPrivate.get(`/ticket-updates/admin/view/${id}`);
            // console.log(response)
    ...
    const ticketStatusHandler = async (e) => {
        let value = e.target.value;
        showLoader()
        try {

            const response = await axiosPrivate.post(`/ticket/admin/status/${id}`, { status: value },);
            // console.log(response)
            ...
            if (err?.response?.data?.errors?.status) {
                errorToast(err?.response?.data?.errors?.status?.msg)
            }
    ...
    const newTicketUpdateHandler = async (event) => {
        ...
        try {
            ...
            const response = await axiosPrivate.post(`/ticket-updates/admin/create/${id}`, { message },);
            // console.log(response)
            // successToast(response.data.message)
            notificationHandler({
    ...
    const notificationHandler = async (data) => {
        ...
        try {
            ...
            const response = await axiosPrivate.post(`/notification/admin/create/${id}`, data,);
            // console.log(response)
            // socket.emit('send message', {roomId:id,data:response?.data?.data});
    ...
    return (
        <InnerDashboardLayout>
            <DashboardHeader name="View Ticket" link={`/admin/edit-ticket/${id}`} routeName="Edit Ticket" />
                    ...
                    <Table responsive>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <select className="form-control" value={status} onChange={(e) => ticketStatusHandler(e)}>
